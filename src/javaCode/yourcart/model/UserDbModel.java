...
import javaCode.yourcart.utilize.SHA;
    ...
    public boolean signUp(User bean) {
        ...
        if (!search(bean.getUserName())) {
            try {
                ...
                PreparedStatement pst1 = con.prepareStatement("INSERT into users (username,email,address, password,job,creaditCard,cash,role,phone_number)"
                        + "values (?,?,?,?,?,?,?,?,?)");
                ...
                pst1.setString(9,bean.getPhoneNumber());
    ...
    public boolean updateUser(User updateUser ,String path) {
        ...
        try {
            ...
            if (search(updateUser.getUserName())) {
                ...
                {
                     boolean deleteFile = FileUpload.deleteFile(temp.getPhoto(), path);
